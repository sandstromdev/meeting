...
import { ErrorMessages } from '@lsnd-mt/common/errorMessages';
import { isAppErrorCode } from '@lsnd-mt/common/appError';
import type { AppErrorCode, AppErrorPayloadFor } from '@lsnd-mt/common/appError';
export type { AppErrorCode, AppErrorPayloadFor } from '@lsnd-mt/common/appError';
export { isAppErrorCode } from '@lsnd-mt/common/appError';
import { z } from 'zod';
...
export class AppError<
	C extends AppErrorCode = AppErrorCode,
	P extends AppErrorPayloadFor<C> = AppErrorPayloadFor<C>,
> extends ConvexError<AppConvexErrorData> {
	constructor(code: C, status: number, data?: P) {
		...
		this.message = ErrorMessages[code]((data ?? {}) as AppErrorPayloadFor<C>);
	}
	...
	static fromConvex<P extends {}>(err: ConvexError<AppErrorStored<string, P>>) {
		...
		if (!isAppErrorCode(code)) {
			return appErrors.internal_error();
		}
		return new AppError(code, status, (data ?? {}) as AppErrorPayloadFor<typeof code>);
	}
...
export const appErrors = {
	...
	bad_request: (args: Record<string, Value | undefined>) =>
		new AppError('bad_request', 400, { args: args as Record<string, unknown> }),
