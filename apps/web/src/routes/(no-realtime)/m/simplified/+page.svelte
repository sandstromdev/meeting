<script lang="ts">
	import { computeAgendaNumbers } from '@lsnd-mt/convex/helpers/agenda';
	...
	import type { SimplifiedColdSnapshot } from '@lsnd-mt/convex/meeting/users/simplified';
	...
	type AgendaRow = SimplifiedColdSnapshot['agenda'][number];

	const simplifiedAgendaFlat = $derived(
		computeAgendaNumbers(
			(p.coldSnapshot?.agenda ?? []).map((a: AgendaRow) => ({
				id: a.id,
				title: a.title,
