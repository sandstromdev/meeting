...
| Path                                 | Package             | Role                                                                                                                 |
| ------------------------------------ | ------------------- | -------------------------------------------------------------------------------------------------------------------- |
| [`apps/meetings`](apps/meetings)     | `@lsnd-mt/meetings` | SvelteKit meeting and poll-by-code UI (default dev server **port 4000**)                                             |
| [`apps/polls`](apps/polls)           | `@lsnd-mt/polls`    | SvelteKit polls app shell; same toolchain as meetings (**port 4001**)                                                |
...
- **Where it lives**:
  - **Primary**: `apps/meetings/src/routes/(realtime)/m/...`
  - **Simplified HTTP fallback**: `apps/meetings/src/routes/(no-realtime)/m/simplified/`
...
- **Standalone entrypoint**: `/p/[code]` (see `apps/meetings/src/routes/(no-realtime)/p/[code]/+page.svelte`). A separate [`apps/polls`](apps/polls) SvelteKit package exists for future poll-focused surfaces; it shares the same Convex workspace and env conventions as meetings.
...
```sh
cd apps/meetings && bun run dev
```

```sh
cd apps/polls && bun run dev
```
...
Meetings is usually served at `http://localhost:4000` and polls at `http://localhost:4001` (see each package’s `dev` script).

### 4) Build, check, test (root)

```sh
bun run build    # Turbo: production build (SvelteKit apps + any other build tasks)
bun run check    # Turbo: svelte-check (meetings, polls, …)
bun run test     # Turbo: Vitest (apps + convex)
...
Environment variables are described with **[Varlock](https://varlock.dev)** using **[@env-spec](https://varlock.dev/env-spec)** schema files: a single canonical spec in [`packages/common/.env.schema`](packages/common/.env.schema), imported by [`apps/meetings/.env.schema`](apps/meetings/.env.schema), [`apps/polls/.env.schema`](apps/polls/.env.schema), [`apps/convex/.env.schema`](apps/convex/.env.schema) (local Convex / CLI), and [`apps/convex/.env.remote.schema`](apps/convex/.env.remote.schema) (dashboard env checks). Those files drive docs and Varlock checks only—**SvelteKit** still loads `.env*` from each app directory (`apps/meetings`, `apps/polls`), and **Convex** reads `process.env` from the [Convex dashboard](https://dashboard.convex.dev) (keep dashboard values aligned with local secrets where the backend needs them).

**`apps/convex/.env.local`** is written when you run **`bunx convex dev`** from **`apps/convex`**; extend it with any other keys the schema requires. **`apps/meetings/.env.local`** and **`apps/polls/.env.local`** are per frontend—create them as needed and keep shared values aligned with Convex where both apps need them.
...
**Per app** (from repo root): `bun run env:validate -- --filter=@lsnd-mt/meetings`, `bun run env:validate -- --filter=@lsnd-mt/polls`, or `bun run env:validate -- --filter=@lsnd-mt/convex` (same for `env:load`), or `cd` into an app package and run the same scripts there.
...
Each SvelteKit app is configured for Vercel with its own **Root Directory** (`apps/meetings`, `apps/polls`). See [`apps/meetings/vercel.json`](apps/meetings/vercel.json) and [`apps/polls/vercel.json`](apps/polls/vercel.json): install runs from the **monorepo root** (`cd ../.. && bun install --frozen-lockfile`) so workspace dependencies resolve. Production deploys that bundle Convex use each app’s `build:vercel` script (Convex deploy + that app’s Vite build).

## Repo quick tour

- **Routes (meetings UI)**: `apps/meetings/src/routes/`
  - Primary meeting UI: `apps/meetings/src/routes/(realtime)/...`
  - HTTP-only simplified UI: `apps/meetings/src/routes/(no-realtime)/m/simplified/`
- **Polls app (scaffold)**: `apps/polls/src/routes/`
- **Backend (Convex)**: `apps/convex/src/`
- **Shared TS + env spec**: `packages/common/src/`, `packages/common/.env.schema`
- **UI primitives**: `apps/meetings/src/lib/components/ui/`
