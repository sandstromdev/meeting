...
After changing `.env.schema`, regenerate TypeScript types: `bun run env:typegen` (also runs during `bun install` via `prepare`, at the start of `bun run check` / `bun run build`). Output is `src/env-varlock.d.ts` (gitignored).

Check required variables resolve: `bun run env:validate` (also runs at the start of `bun run build`).
