<script lang="ts">
	...
	let {
		...
		showDiscard = false,
		onDiscard,
		discardLabel = 'Kassera ändringar',
	}: Props = $props();
	...
	let localVisibilityMode = $derived(isStandalone ? visibilityMode : 'public');
...
<Card.Root class="has-[[data-slot=field-error]]:border-destructive/50">
	<Card.Header>
		<Card.Title>{title}</Card.Title>
		<Card.Description>Redigera omröstningen.</Card.Description>
	</Card.Header>
	<Card.Content>
								...
								<FormControl>
									{#snippet children({ props })}
										...
										<NumberField.Root
											min={1}
											max={optionsCount}
											bind:value={$draft.maxVotesPerVoter}
										>
											<NumberField.Group class="max-w-40">
												<NumberField.Decrement />
												<NumberField.Input {...props} />
